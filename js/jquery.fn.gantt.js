		...
		var methods = {
			init : function(options) {
				...
				return this.each(function() {
					...
					// How many rows on last page
					$this.rowsNum = 0;
					// Number of total rows
					$this.hPosition = 0;
					// Current position on diagram (Horizontal)
					$this.dateStart = null;
					$this.dateEnd = null;
		...
		var core = {
			...
			init : function(element) {
				...
				if (!element.data || !element.data.length) {
					element.rowsNum = 0;
					element.pageCount = 1;
					element.rowsOnLastPage = 0;

					var now = new Date();
					element.dateStart = tools.getScaledMinDate(element, now);
					element.dateEnd = tools.getScaledMinDate(element, now);
				} else {
			...
			leftPanel : function(element) {
				...
				/* Left panel */
				var ganttLeftPanel = $('<div class="leftPanel"/>').append($('<div class="row spacer"/>').css("height", tools.getCellSize() * element.headerRows + "px").css("width", "100%"));
			...
			navigation : function(element) {
				...
				if (settings.navigate === "scroll") {
					...
					})).append($('<div class="page-number"/>').append($('<span/>').html(element.pageNum + 1 + ' of ' + element.pageCount))).append($('<span role="button" class="nav-link nav-page-next"/>').html('&gt;').click(function() {
						core.navigatePage(element, 1);
			...
			waitToggle : function(element, show, fn) {
				...
				if (show) {
					...
					if (!element.loader) {
						element.loader = $('<div class="fn-gantt-loader" style="position: absolute; top: ' + eo.top + 'px; left: ' + eo.left + 'px; width: ' + ew + 'px; height: ' + eh + 'px;">' + '<div class="fn-gantt-loader-spinner"><span>' + settings.waitText + '</span></div></div>');
					}
